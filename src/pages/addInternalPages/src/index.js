const addPagesToListBtn = document.getElementById('add-to-list-btn');
const getPages = document.getElementById('request-content-btn');
const download = document.getElementById('download');
const downloadImgs = document.getElementById('download-images');

let pages = [], allContent = '', allImgSrc = [];

addPagesToListBtn.onclick = () => {
    if(document.getElementById('input-pages').value.split(' ').length > 1){
        document.getElementById('input-pages').value.split(' ').forEach((urls) => {
            pages.push(urls);
        }) 

    }else{
        pages.push(document.getElementById('input-pages').value);
    }
    ...
    document.getElementById("list-container").appendChild(listOfPages);
}

getPages.onclick = () => {
    let filter = document.getElementById('content-filter').value;
    let requestDone = 0;
    pages.forEach((url) => {
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                document.getElementById('js-request-content').innerHTML += `
    ...
    setTimeout(() => {
        allImgSrc.forEach((source) => {
            ...
            .then(blob => {
                ...
                window.URL.revokeObjectURL(url);
                imagesDone++;
                document.getElementById('images-done').innerHTML = `Images downloaded ${imagesDone}/${allImgSrc.length} `
            })
        })
      }, "1000");
